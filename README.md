...
| `Cr2O72-` | $\text{Cr}_2\text{O}_7^{2-}$ | Deep subscripts + anionic charges |
| `Ca(OH)2` | $\text{Ca(OH)}_2$ | Nested compound brackets |
| `[Co(NH3)6]Cl3` | $\text{[Co(NH}_3\text{)}_6\text{]Cl}_3$ | Complex coordination compound |
| `CuSO4*5H2O` | $\text{CuSO}_4 \cdot 5\text{H}_2\text{O}$ | Hydration state using `*` spacing |
| `Fe^3+` | $\text{Fe}^{3+}$ | Explicit caret charge mapping |
| `H2O(l)` | $\text{H}_2\text{O}\text{(l)}$ | State of matter detection |
| `H2 + O2 --> H2O` | $\text{H}_2 \text{ + } \text{O}_2 \longrightarrow \text{H}_2\text{O}$ | Full chemical reaction mapping |
...
| `dy/dx` | $\frac{dy}{dx}$ | Calculus derivatives |
| `sqrt(x)` | $\sqrt{x}$ | Roots |
| `root(3, x)` | $\sqrt[3]{x}$ | N-th Root |
| `x^y` | $x^y$ | Powers / Superscripts |
| `x_i` | $x_i$ | Subscripts |

### 3. Quick-type Operators & Greek Letters

* **Symbols:** `pi` $\rightarrow$ $\pi$ | `theta` $\rightarrow$ $\theta$ | `lambda` $\rightarrow$ $\lambda$ | `ohm` $\rightarrow$ $\Omega$ | `micro` $\rightarrow$ $\mu$ | `deg` $\rightarrow$ $^\circ$ | `inf` $\rightarrow$ $\infty$
* **Comparisons:** `!=` $\rightarrow$ $\neq$ | `<=` $\rightarrow$ $\leq$ | `>=` $\rightarrow$ $\geq$ | `approx` $\rightarrow$ $\approx$
* **Arrows:** `->` $\rightarrow$ $\rightarrow$ | `-->` $\rightarrow$ $\longrightarrow$ | `<=>` $\rightarrow$ $\rightleftharpoons$ | `=>` $\rightarrow$ $\Rightarrow$
